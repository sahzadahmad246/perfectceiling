...
export async function POST(req: NextRequest) {
  ...
  payload.updatedByUserId = (session as any).user?.id;
...
export async function PUT(req: NextRequest) {
  ...
  if (contentType.includes("multipart/form-data")) {
    const form = await req.formData();
    if (form.has("name")) payload.name = form.get("name");
    if (form.has("primaryPhone")) payload.primaryPhone = form.get("primaryPhone");
    if (form.has("secondaryPhone")) payload.secondaryPhone = form.get("secondaryPhone") || undefined;
    if (form.has("status")) payload.status = form.get("status");
    const terms = form.getAll("terms[]").map((t) => String(t)).filter(Boolean);
