...
import { Layers, Home, ChevronRight, ArrowRight, Briefcase } from "lucide-react"
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (currentCategory) {
          ...
          setSubs((subcategories as SubcategoryDTO[]).filter((s) => s.categoryId === categoryId))
          setServices((services as ServiceDTO[]).filter((s) => s.categoryId === categoryId))
        } else {
          setSubs([])
          setServices([])
        }
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div className="mb-8 sm:mb-12">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-4 sm:mb-6 px-1">
            Browse Services by Subcategories
          </h2>
          <div className="grid gap-3 sm:gap-4 lg:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {subs.map((sub) => {
              const subId = sub.id
              const subServices = services.filter((s) => s.subcategoryId === subId)
              return (
                <Link href={`/services/${category}/${sub.slug}`} key={subId} className="group block">
                  <div className="h-full bg-white/80 backdrop-blur-sm border border-slate-200 rounded-xl sm:rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] hover:border-blue-300">
                    {sub.image?.url && (
                      <img
                        src={sub.image.url || "/placeholder.svg"}
                        alt={sub.name}
                    ...
                    <div className="p-3 sm:p-4 lg:p-6">
                      <div className="flex items-start gap-2.5 sm:gap-3 lg:gap-4 mb-3 sm:mb-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 mt-1">{subServices.length} services</p>
                        </div>
                      </div>

                      <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-slate-100">
                        <span className="text-xs sm:text-sm text-slate-500">Explore services</span>
          ...
          <div>
            ...
            <div className="grid gap-3 sm:gap-4 lg:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {services.map((service) => {
                ...
                const subcategory = subs.find((s) => s.id === service.subcategoryId)
                const imageUrl = service.images?.[0]?.url || subcategory?.image?.url || cat.images?.[0]?.url
                return (
                  <Link
                    href={`/services/${category}/${subcategory?.slug}/${service.slug}`}
                    key={serviceId}
                      ...
                      <div className="p-3 sm:p-4 lg:p-6">
                        ...
                        {subcategory && (
                          <div className="mb-3 sm:mb-4">
                            <span className="inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700 border border-indigo-200">
                              {subcategory.name}
                            </span>
                          </div>
                        )}
        ...
        {subs.length === 0 && (
          <div className="text-center py-12 sm:py-16 px-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center mx-auto mb-3 sm:mb-4">
              <Layers className="h-6 w-6 sm:h-8 sm:w-8 text-slate-400" />
            </div>
            <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-2">No subcategories available</h3>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            hasPart: subs.map((sub) => ({
              "@type": "ItemList",
              name: sub.name,
              url: `/services/${category}/${sub.slug}`,
            })),
          }),
        }}
      />
