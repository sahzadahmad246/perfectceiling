...
type QuotationItemImageViewerProps = {
  open: boolean;
...
export function QuotationItemImageViewer({
  open,
  images,
  initialIndex = 0,
  title,
  onClose,
}: QuotationItemImageViewerProps) {
  ...
  const description = activeImage.description.trim();
  ...
  return (
    <div className="fixed inset-0 z-[10030] h-dvh w-full overflow-hidden bg-black/92 backdrop-blur-md">
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/80 via-black/50 to-transparent px-4 pb-5 pt-10 sm:px-6 sm:pb-6">
