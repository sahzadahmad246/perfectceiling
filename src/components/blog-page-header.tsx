...
import { getBlogPageBreadcrumb } from "@/lib/admin-nav";

type BlogPageHeaderProps = {
  onAddPost: () => void;
};

export function BlogPageHeader({ onAddPost }: BlogPageHeaderProps) {
  return (
    <div className="sticky top-0 z-20 -mx-4 bg-surface/90 backdrop-blur-xl sm:-mx-8">
      ...
      <AdminDetailBreadcrumbBar items={getBlogPageBreadcrumb()} />
    </div>
  );
}