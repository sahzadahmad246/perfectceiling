...
import { getUnitLabel } from "@/lib/quotations";
...
const UNIT_OPTIONS: { value: InvoiceUnitType; label: string }[] = [
...
type InvoiceItemModalProps = {
  open: boolean;
  initialItem?: InvoiceLineItemDraft | null;
  onClose: () => void;
  onSave: (item: InvoiceLineItemDraft) => void;
};
...
export function InvoiceItemModal({
  ...
  const [item, setItem] = useState<InvoiceLineItemDraft>(createEmptyInvoiceLineItem);
  ...
  useEffect(() => {
    ...
    setItem(initialItem ? { ...initialItem } : createEmptyInvoiceLineItem());
    setErrors({});
  }, [initialItem, open]);
  ...
  function updateItem(patch: Partial<InvoiceLineItemDraft>) {
